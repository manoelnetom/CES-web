...
from .models import Objeto, Movimentacao, GrupoObjeto, TipoObjeto
...
class FazerReservaListView(LoginRequiredMixin, generic.ListView):
    ...
    def dispatch(self, *args, **kwargs):
        ...
        try:
            self.tipoobjeto_id = kwargs['tipo']
        except KeyError:
            pass
    ...
    def get_context_data(self, **kwargs):
        context = super(FazerReservaListView, self).get_context_data(**kwargs)
        try:
            if self.tipoobjeto_id:
                objeto = get_object_or_404(TipoObjeto, pk=self.tipoobjeto_id)
                context['current_tipo'] = objeto
    ...
    def get_queryset(self):
        ...
        try:
            if self.tipoobjeto_id:
                objetos = objetos.filter(tipoObjeto__id=self.tipoobjeto_id)
        except AttributeError:
            pass
...
class ReservaCreateView(LoginRequiredMixin, generic.CreateView):
    ...
    success_url = reverse_lazy('fazer_reserva')
